...
#python -m streamlit run "C:\Users\eacou\OneDrive - Bentley University\[1] Portfolio\Portfolio\erin_portfolio.py"
...
if page_select == "Resume":
    ...
    #st.image(image, caption="Erin Courtemanche Resume Preview", use_container_width=True)

    buffered = BytesIO()
    image.save(buffered, format="PNG")
    encoded_resume = base64.b64encode(buffered.getvalue()).decode()

    # Display resume with border
    st.markdown(
        f"""
        <div style="border: 4px solid #FF4B4B; border-radius: 12px; padding: 10px; display: inline-block;">
            <img src="data:image/png;base64,{encoded_resume}" width="100%" />
            <p style="text-align: center; font-weight: bold; margin-top: 8px;">Erin Courtemanche Resume Preview</p>
        </div>
        """,
        unsafe_allow_html=True
    )
...
if page_select == "Projects":
    ...
    startup_pdf_path = Path(__file__).parent / "business_start_time.pdf"
    with open(startup_pdf_path, "rb") as f:
        st.download_button(
    ...
    ticket_pdf_path = Path(__file__).parent / "gross_ticket_sales.pdf"
    with open(ticket_pdf_path, "rb") as f:
        st.download_button(
            label="📥 Download Theater Ticket Sales Project PDF",
            data=f,
